...
- 推荐 YOLOv8n ONNX + OpenCV DNN，不安装 PyTorch/ONNX Runtime；没有模型时自动回退到 OpenCV HOG，方便先打通流程。
...
脚本会在一次性容器中导出 `models/yolov8n.onnx`。重新启动服务后，`DETECTOR_MODE=auto` 会自动使用它。也可以自行导出兼容的 COCO YOLOv8 检测模型放到同一路径。
...
座位 ROI 初始为画面中央 70% 宽、80% 高。摄像头固定后，应根据机位调整数据库设置中的 `roi_x/y/w/h`（均为 0–1 比例）；后续可以在网页设置中继续扩展可视化框选。
