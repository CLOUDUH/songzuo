...
- Bark Server、设备 Key、Webhook、提醒文案和汇总时间均可在 Web 设置；支持首次久坐提醒、按实际坐姿时长自定义 5–360 分钟的重复提醒间隔、每日小结和每周报告。
...
摄像头离线、画面超过 30 秒未更新、解码失败及服务重启会单独记录为“监测中断”。
中断期间状态未知，不计入坐姿或离座时间；恢复后需重新确认在座并开启新会话。
首页和按日查询的时间轴以灰色标识离线／状态未知区间，按日查询保留中断明细；日报和周报的坐姿统计也排除中断时间。
因旧版本没有逐帧健康记录，历史故障的真实恢复时间无法自动还原。

已确认的历史异常会话可在容器内使用 `python -m app.repair_interruption --session-id ID --expected-start ISO时间` 重新分类。
工具仅处理已结束且 ID、开始时间同时匹配的单条会话，先把会话、离座记录和提醒日志备份进 `repair_backups`，再移出坐姿统计。
返回的备份键可用于 `python -m app.repair_interruption --restore BACKUP_KEY` 恢复。
历史重分类区间标为“状态未知”，不表示这段时间的每一秒都已证实网络离线。
